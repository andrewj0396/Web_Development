#!/usr/bin/python3
#This script populates the content of an html file

import re
from bs4 import BeautifulSoup

#Initialize empty todo_list
todo_list = []

#Read the contents of todo list text file todo_list
with open("todo's.txt") as r_file:
    todo = r_file.readline()
    while todo:
        todo = todo.strip()
        todo_list.append(todo)
        todo = r_file.readline()

#Open and read from the index HTML file.
with open("index.php") as index:
    index_html = index.read()

#First check if there is already a set of list items in there
if "<li>" in index_html:
    #If there are use some regex to make a list of those lines called lines_to_replace
    lines_to_replace = re.findall(r'<li>(.*?)</li>', index_html)

#Take the elements of lines_to_replace, reformat them, then replace them within the index_html text string
for line in lines_to_replace:
    #Add <li> and </li> tags to line (the regex above doesn't incluede those)
    newLine = "<li>{:s}</li>".format(line)
    print(newLine)
    #Replaces the top line with the HTML comment tag
    if lines_to_replace.index(line) == 0:
        index_html = index_html.replace(newLine, "<!--list-->")
    #Replaces all other lines with an empty string
    elif lines_to_replace.index(line) > 0:
        index_html = index_html.replace(newLine, "")
print(index_html)

#Prepare string of HTML list items to be injected into html file b/w <ul> tags
html_list = ''

for todo in todo_list:
    #print(type(todo_list.index(todo)))
    html_list += '<li>{:s}{:s}<input type="checkbox" name="Test{:d}[]" value="on"></li>'.format(todo, ":", todo_list.index(todo))
    html_list += '\n'
#print(html_list)

#Replace the empty <ul> tags in the index_html with the newly formatted string
index_html = index_html.replace("<!--list-->", html_list)
print(index_html)
'''
#Write the changed html code back to the index.php file
with open("index.php", "w") as open_file:
    open_file.write(index_html)
'''
